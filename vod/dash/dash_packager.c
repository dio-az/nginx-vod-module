...
dash_packager_get_frame_pres_time(
	media_set_t* media_set, media_track_t* track, uint64_t clip_pres_time, uint64_t dts, uint32_t pts_delay
) {
...
dash_packager_get_earliest_pres_time(media_set_t* media_set, media_track_t* track) {
	...
	return dash_packager_get_frame_pres_time(
		...
		track->frames.first_frame[0].pts_delay
	);
}
...
static size_t
dash_packager_get_styp_sidx_atoms_size(media_set_t* media_set, bool_t large_timestamps) {
	return (media_set->version >= 2 ? sizeof(styp_atom_v2) : sizeof(styp_atom))
	     + ATOM_HEADER_SIZE
	...
	for (cur_item = segment_durations->items; cur_item < last_item; cur_item++) {
		...
		for (; segment_index < last_segment_index; segment_index++) {
			...
			while (dts < segment_end_dts) {
				...
				if (frame_count == 0) {
					...
					earliest_pres_time = dash_packager_get_frame_pres_time(
						media_set, track, clip_pres_time, dts, cur_frame->pts_delay
					);
				}
			...
			callback(
				...
				dash_packager_get_styp_sidx_atoms_size(media_set, earliest_pres_time > UINT_MAX),
				dash_packager_get_moof_atom_size(
...
dash_packager_build_fragment_header(
	...
	*total_fragment_size =
		dash_packager_get_styp_sidx_atoms_size(media_set, sidx_params.earliest_pres_time > UINT_MAX)
		+ moof_atom_size
		+ mdat_atom_size;
