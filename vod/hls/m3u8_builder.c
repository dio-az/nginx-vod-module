...
m3u8_builder_build_iframe_playlist(
	...
	rc = m3u8_builder_build_tracks_spec(request_context, media_set, suffix, &ctx.name_suffix);
	if (rc != VOD_OK) {
		return rc;
	}
...
m3u8_builder_write_iframe_variants(
	...
	for (cur_track_ptr = adaptation_set->first; cur_track_ptr < adaptation_set->last; cur_track_ptr += muxed_tracks) {
		...
		if (m3u8_builder_is_fmp4_container(conf->container_format, HLS_ENC_NONE, video->codec_id)) {
			if (adaptation_set->type == ADAPTATION_TYPE_MUXED
			    || !media_set->segmenter_conf->align_to_key_frames) {
				continue;
			}
		} else if (!mp4_to_annexb_simulation_supported(video)) {
			continue;
		}
